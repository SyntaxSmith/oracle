import type { ChatGptApiRunResult } from "./types.js";
...
function applyPatchFrame(frame: Record<string, unknown>, state: AggregatorState): void {
  ...
  // Bootstrap: `{v: {message, conversation_id}}` — seed state from inner shape.
  if (typeof v === "object" && v !== null && !Array.isArray(v) && p === undefined) {
  ...
  // Batched patches: `{v: [<patch1>, <patch2>, ...]}`.
  if (Array.isArray(v) && p === undefined) {
    for (const sub of v) {
...
function captureFromMessage(
  ...
  if (typeof message.status === "string" && !state.finishReason) {
